...
| 内部 role（不变） | 对外名称 | 主要 `@` 标签 |
|-------------------|----------|----------------|
| 产品经理 | **产品经理** | `@产品经理`（仍兼容 `@需求顾问`、`@PM`、`@小k` 等） |
| 实习生 | **实习生** | `@实习生`（仍兼容 `@代码助手`、`@intern` 等） |
| 老师傅 | **老师傅** | `@老师傅`（仍兼容 `@代码生成`、`@master` 等） |
...
- 在对话栏标题区域下方增加 **三个按钮**，分别插入 `@产品经理 `、`@实习生 `、`@老师傅 `，并聚焦输入框。
...
- 在以下路径中设置/清除 `busyAgentRole`，使对应按钮出现 **高亮/脉冲动画**（如 `is-working`、小圆点）：
  - 需求确认对话 `handleConfirmationChat`（按 `@` 解析角色，无 `@` 时默认产品经理）
  - 确认后走生成 `handleGenerateAfterConfirmation`（老师傅生成）
  - 首次方案+生成 `handlePlanThenGenerate`、代码修订 `handleRevisionGenerate`（老师傅生成）
...
- 当前：**工程师类消息**（`role: 'engineer'`）仍显示与 `agentRole` 对应的头像（如实习生/老师傅）。
